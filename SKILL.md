...
### Serena 代码分析工具
- `serena_init` - 初始化 Serena 客户端
- `serena_status` - 获取 Serena 状态
- `serena_symbols` - 获取文件符号概览
- `serena_find_symbol` - 查找符号
- `serena_find_refs` - 查找符号引用
- `serena_search` - 正则搜索代码
- `serena_find_file` - 查找文件
- `serena_rename` - 重命名符号
- `serena_replace_body` - 替换符号体
...
### serena_init
初始化 Serena 客户端，检查 Serena 是否可用并初始化项目。

参数:
- `project_path` (string, required): 项目路径
- `language_backend` (string, default: "LSP"): 语言后端 (LSP/JETBRAINS)
...
```json
{
  ...
  "available_tools": ["find_symbol", "get_symbols_overview", ...]
}
```
...
参数:
- `relative_path` (string, required): 相对文件路径
- `depth` (integer, default: 0): 符号深度

返回:
```json
{
  "Class": [{"name_path": "MyClass", "kind": "Class", ...}],
  "Function": [{"name_path": "main", "kind": "Function", ...}],
  "Variable": [...]
}
```
...
参数:
- `name_path_pattern` (string, required): 符号路径模式
- `relative_path` (string, optional): 相对文件路径
- `depth` (integer, default: 0): 获取子节点的深度
- `include_body` (boolean, default: false): 是否包含符号体代码
...
参数:
- `name_path` (string, required): 符号的完整路径
- `relative_path` (string, required): 包含该符号的文件路径

返回:
```json
[
  {"name": "usage", "relative_path": "src/other.py", "line": 20}
]
```
...
参数:
- `substring_pattern` (string, required): 正则表达式
- `relative_path` (string, optional): 限制搜索范围
- `paths_include_glob` (string, optional): 包含文件glob模式
- `paths_exclude_glob` (string, optional): 排除文件glob模式
- `context_lines_before` (integer, default: 0): 前置上下文行数
- `context_lines_after` (integer, default: 0): 后置上下文行数

返回:
```json
{
  "src/main.py": [
    {"line": 10, "content": "def initialize():", "match": "initialize"}
  ]
}
```
...
参数:
- `file_mask` (string, required): 文件名或通配符
- `relative_path` (string, default: "."): 相对路径
