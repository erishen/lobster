...
def init(project_path):
    ...
    if not client.is_available():
        logger.error("Error: Serena 未安装或未配置")
        logger.info("请设置 SERENA_DIR 环境变量:")
        logger.info(" export SERENA_DIR=/path/to/serena")
        return
    ...
    if result.get("success"):
        logger.info(" Serena 初始化成功")
        logger.info(f" 项目路径: {result.get('project')}")
        logger.info(f" 语言后端: {result.get('language_backend')}")
        logger.info(f" 可用工具: {len(result.get('available_tools', []))} 个")
    else:
        ...
        for kind, symbols in result.items():
            if symbols:
                ...
                for s in symbols[:10]:
                    ...
                    logger.info(f" • {name}" + (f" (L{line})" if line else ""))
    else:
...
def find(symbol_name, file_path, body):
    ...
    if isinstance(result, list):
        for i, s in enumerate(result[:5], 1):
            ...
            logger.debug(f"\n{i}. {name} ({kind})")
            logger.info(f" 文件: {path}" + (f" (L{line})" if line else ""))
    ...
    if isinstance(result, dict):
        for file_path, matches in list(result.items())[:10]:
            ...
            for m in matches[:5]:
                ...
                logger.info(f" L{line}: {content[:100]}")
    else:
        logger.error(f"Error: {result}")
        ...
        if isinstance(files, list):
            for f in files[:20]:
                if isinstance(f, dict):
                    logger.info(f" • {f.get('path', f.get('name', str(f)))}")
                else:
                    logger.info(f" • {f}")
        else:
...
def refs(symbol_name, file_path):
    ...
    if isinstance(result, list):
        for ref in result[:20]:
            ...
            line = ref.get("line", "")
            logger.info(f" • {name} in {path}" + (f" (L{line})" if line else ""))
    else:
        logger.error(f"Error: {result}")
