...
def todo(path):
    ...
    """
    console.print(Panel(f"📋 [bold cyan]待办事项管理[/bold cyan]", border_style="blue"))
...
@click.option('--priority', '-p', default='medium', help='优先级 (high/medium/low)')
@click.option('--path', '-p', default='.', help='项目路径')
def add_todo(content, priority, path):
    ...
    """
    console.print(Panel(f"➕ [bold cyan]添加待办事项[/bold cyan]", border_style="blue"))
...
def done_todo(todo_id, path):
    ...
    """
    console.print(Panel(f"✅ [bold cyan]完成待办事项[/bold cyan]", border_style="blue"))
